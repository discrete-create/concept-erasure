...
import torch
import numpy as np
def image2latent(vae, image, device, dtype=torch.float16):
    """
    将图像编码为latent向量
    Args:
        vae: 训练好的VAE模型
        image: 输入图像，np.ndarray (H, W, C) 或 torch.Tensor (C, H, W) 或 (B, C, H, W)
        device: torch.device
        dtype: torch.dtype
    Returns:
    ...
    if isinstance(image, np.ndarray):
        ...
        raise TypeError("输入图像必须是 np.ndarray 或 torch.Tensor")

    # 转到 [−1,1]
    image = image.to(dtype=dtype) / 255.0
    image = image * 2 - 1

    # move/cast image to the same device and dtype as the VAE parameters to avoid
    # mixed-type operations (e.g. conv bias float vs input half) which raise runtime errors
    vae_device = next(vae.parameters()).device if any(True for _ in vae.parameters()) else torch.device("cpu")
    try:
        vae_dtype = next(vae.parameters()).dtype
    except StopIteration:
        vae_dtype = image.dtype
...
def generate_imag_with_b_net(
    ...
    return latents
'''
def mask_to_latent_mask(mask_pil, latent_h, latent_w, device, dtype):
    """
    mask_pil: PIL Image, L mode, 0/255
    returns: [1, 1, latent_h, latent_w]
    """
    import torch
    import torch.nn.functional as F
    from torchvision import transforms

    mask = transforms.ToTensor()(mask_pil)  # [1, H, W], 0~1
    mask = mask.unsqueeze(0).to(device=device, dtype=dtype)  # [1,1,H,W]

    mask = F.interpolate(
        mask,
        size=(latent_h, latent_w),
        mode="nearest"   # ❗ 必须是 nearest
    )
...
def generate_step_with_mask(
    ...
    """
    Single denoising step that ensures intermediate tensors are created with `dtype`.
    Applies mask to control which regions are updated.

    Args:
        noise_pred: predicted noise εθ(x_t)
        sample: current latent x_t
        mask: binary mask tensor of shape (1, 1, latent_h, latent_w)
              0 = keep original (lock)
              1 = update via diffusion
    """
    if device is None:
        device = sample.device

    # -----------------------------
    # 1. compute previous timestep
    # -----------------------------
    step = scheduler.config.num_train_timesteps // scheduler.num_inference_steps
    prev_timestep = max(0, timestep - step)
    ...
    alpha_prev = alphas[prev_timestep]
    ...
    # -----------------------------
    # 3. predict x0 (DDIM-style)
    # -----------------------------
    pred_x0 = (sample - sqrt_one_minus_alpha_t * noise_pred) / sqrt_alpha_t

    # -----------------------------
    # 4. compute direction to x_{t-1}
    # -----------------------------
    pred_dir = sqrt_one_minus_alpha_prev * noise_pred

    # -----------------------------
    # 5. compute x_{t-1}
    # -----------------------------
    prev_sample = sqrt_alpha_prev * pred_x0 + pred_dir

    # -----------------------------
    # 6. apply mask (核心)
    # -----------------------------
    # mask == 0 → keep original sample
    # mask == 1 → use diffusion update
    prev_sample =scheduler.add_noise(original_latents, noise, timestep) * (1 - mask) + prev_sample * mask
...
def generate_img_with_mask(
        latents, prompts,negative_prompt, tokenizer, text_encoder, unet, scheduler, device,
        mask, guidance_scale=7.5, num_inference_steps=20, eta=0.0, dtype=torch.float16):
    """
    Generate latents using classifier-free guidance and a mask.
    ...
    cond_ids = tokenizer(prompts, padding="max_length", max_length=77,
                         truncation=True, return_tensors="pt").input_ids.to(device)
    uncond_ids = tokenizer([negative_prompt] * batch_size, padding="max_length", max_length=77,
                           truncation=True, return_tensors="pt").input_ids.to(device)
    with torch.no_grad():
        cond_emb = text_encoder(cond_ids)[0].to(dtype=dtype, device=device)
        uncond_emb = text_encoder(uncond_ids)[0].to(dtype=dtype, device=device)
    ...
    t_start_index = int(len(timesteps) * 0.7)
    t_start = timesteps[t_start_index]
    noise = torch.randn_like(latents)
    noisy_latents = scheduler.add_noise(latents, noise, t_start)

    latents = latents * (1 - mask) + noisy_latents * mask
    timesteps = timesteps[t_start_index:] 
    # ============================================================
    #  Begin denoising
    # ============================================================
    for t in tqdm(timesteps):
        ...
        # mask-aware guidance
        base_guidance = base_guidance * mask + noise_pred_uncond * (1 - mask)

        # ================================
        # 更新 latent，得到 imgs
        # ================================
        latents = generate_step_with_mask(
            noise, 
            latents_orig,
            scheduler, base_guidance, t, latents, mask, eta,
            dtype=dtype, device=device
        )
    return latents  